...
async def save_in_cloud(upload_data, image_type):
    ...
    s3_file_name = ''.join(random.choices(string.ascii_lowercase + string.digits, k=8))+f".{image_type}"
    s3.put_object(Body=upload_data, Bucket=s3_bucket_name, Key=s3_file_name, ContentType=content_type)
